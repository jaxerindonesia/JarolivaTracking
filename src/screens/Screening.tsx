...
export default function Screening() {
  ...
  useEffect(() => {
    api<any>('/screenings/latest').then((saved) => {
      if (!saved) return
      if (saved.score < 60 && (!saved.retry_at || new Date() >= new Date(saved.retry_at))) return
  ...
  if (showResult) {
    ...
    return (
      <div className="screening-page screening-result-page fade-in">
        ...
        <div className="points-banner"><span className="points-confetti">🎉</span><div className="points-banner-text"><strong>{freshResult && result.points_earned > 0 ? `+${result.points_earned} Poin Didapat!` : 'Hasil Screening Tersimpan'}</strong><span>{freshResult ? 'Hasil screening sudah disimpan ke database.' : 'Hasil screening terakhir Anda tetap tersimpan.'}</span></div></div>
        ...
        {passed && <button className="screening-reset" onClick={() => { setForm(initialForm); setStep(1); setFreshResult(false); setShowResult(false) }}>Isi Ulang Screening</button>}
      </div>
    )
  }
