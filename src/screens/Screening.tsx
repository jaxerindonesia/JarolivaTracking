    ...
    <div className="screening-page fade-in">
      ...
      <section className="screening-form-card">
        ...
        <div className="screening-fields">
          ...
          ] as const).map(([name, label, Icon]) => <label key={name}><span><Icon /> {label}</span><Choice name={name} value={form[name]} onChange={setField} /></label>)}<label><span><Stethoscope /> Riwayat penyakit lainnya (opsional)</span><textarea placeholder="Ceritakan kondisi kesehatan lain yang relevan..." value={form.notes} onChange={(e) => setField('notes', e.target.value)} /></label></>}
        </div>
      </section>
